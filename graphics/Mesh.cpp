...
{
    // TODO: convert these to a initialization list instead.
    ...
    if (instancing != 1) {
        ...
        // Makes it so that the transform is only switched when drawing the next instance.
...
)
{
    ...
    if (instancing == 1) {
        ...
        glUniformMatrix4fv(glGetUniformLocation(shader.ID, "translation"), 1, GL_FALSE, glm::value_ptr(translation_));
